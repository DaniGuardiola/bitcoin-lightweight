...
describe('Wallet', () => {
  ...
  describe('Bitcoin electrum BIP39 type', () => {
    ...
    it('derives the correct HD private key from mnemonic', async () => {
      ...
      wallet._hdPrivateKey.constructor.name.should.equal('HDPrivateKey')
      wallet._hdPrivateKey.toString().should.equal(DERIVED_HD_KEY.toString())
    })
    ...
    it('derives the correct HD private key from mnemonic and passphrase', async () => {
      ...
      await wallet._ensureInitialized()
      wallet._hdPrivateKey.constructor.name.should.equal('HDPrivateKey')
      wallet._hdPrivateKey.toString().should.equal(DERIVED_HD_KEY_WITH_PASSPHRASE.toString())
    })
  })
  ...
  describe('Bitcoin electrum BIP32 type', () => {
    ...
    it('stores the correct HD private key', async () => {
      ...
      await wallet._ensureInitialized()
      wallet._hdPrivateKey.constructor.name.should.equal('HDPrivateKey')
      wallet._hdPrivateKey.toString().should.equal(CORRECT_SECRET_KEY)
    })
  })
})
