...
const parser_1 = require("../json-rpc-socket/lib/parser");
...
describe('parser success', () => {
    it('complete chunk done', () => {
        ...
        const f = parser_1.createRecursiveParser('\n');
        ...
        assert(result.code === parser_1.PARSE_STATUS.DONE);
        assert(result.chunk === '');
        assert(max === 0);
        assert(count === 1);
    });
    ...
    it('complete chunks suspend', () => {
        ...
        assert(result.code === parser_1.PARSE_STATUS.SUSPEND);
...
describe('parser error', () => {
    it('error zero depth', () => {
        try {
            parser_1.createRecursiveParser('\n', 0);
        }
        catch (e) {
            return;
        }
